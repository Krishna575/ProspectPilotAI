...
export default function InvestmentIntelligencePage() {
    ...
    try {
      ...
      setResult({
        summary:
          ...
          "Failed to generate investment intelligence.",
        sections: [],
      });
    } finally {
      setIsRunning(false);
  ...
  return (
    <AgentPageShell
      ...
      description="Gain AI-powered investment insights, funding trends, VC interest analysis, and fundraising recommendations."
      color="purple"
      stats={[
        { icon: Globe, label: "Funding Trends" },
        { icon: BarChart3, label: "VC Interest" },
        { icon: TrendingUp, label: "Investment Ready" },
      ]}
    >
      {/* Input Section */}
      <div className="bg-white/[0.03] border border-white/[0.07] rounded-xl p-5 space-y-4">
        <label className="text-sm font-medium text-gray-300">
          Describe your startup idea for investment analysis
        </label>
      ...
      {isRunning && (
        <div className="bg-white/[0.03] border border-white/[0.07] rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-400">
            Agent is analyzing investment opportunities...
          </p>

          <p className="text-xs text-gray-600 mt-1">
            Gemini AI is generating investment insights...
          </p>
        </div>
      )}
