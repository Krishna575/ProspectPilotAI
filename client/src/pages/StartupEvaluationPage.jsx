...
export default function StartupEvaluationPage() {
  ...
  const runAgent = async (q) => {
    ...
    try {
      ...
      console.error("Startup Evaluation Error:", error);

      setResult({
        summary:
          error.response?.data?.message ||
          "Failed to evaluate startup.",
        sections: [],
      });
    } finally {
      setIsRunning(false);
  ...
  return (
    <AgentPageShell
      ...
      description="Evaluate startup viability, innovation, market potential, risks, and investment readiness."
      color="purple"
      stats={[
        { icon: Globe, label: "Innovation Score" },
        { icon: BarChart3, label: "Market Potential" },
    ...
    >
      ...
      <div className="bg-white/[0.03] border border-white/[0.07] rounded-xl p-5 space-y-4">
        <label className="text-sm font-medium text-gray-300">
          Describe your startup idea for evaluation
        </label>
      ...
      {isRunning && (
        <div className="bg-white/[0.03] border border-white/[0.07] rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-400">
            Agent is evaluating your startup...
          </p>
